...
Run as a CLI:
    python scripts/tooling/_next_verified_at_ms.py --passport <path>
        Reads passport YAML/JSON, prints the next verified_at value.

Importable API:
    from scripts.tooling._next_verified_at_ms import next_verified_at_ms
...
def _load_passport_audit_artifacts(path: Path) -> list[dict[str, Any]]:
    ...
    TODO(Phase 6.3): scripts/checks/check_audit_artifact_consistency.py should
