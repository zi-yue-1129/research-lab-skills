...
* ``rq_framing_patterns`` (advisory-calibration) — dispatches to the existing
  ``scripts.checks.check_rq_framing_patterns`` runner and adapts its FNR / FPR /
  balanced-accuracy output into the per-task lift-report shape.

CLI::

    python -m scripts.evals.run_evals [--task <name>] [--baseline <path>]
...
def measure_rq_framing_patterns(task_dir: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    """Dispatch to scripts.checks.check_rq_framing_patterns and adapt its output.
    ...
    """
    from scripts.checks import check_rq_framing_patterns as rq
...
def build_compare_report(baseline: dict[str, Any], compare: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "caveats": [
            ...
            "Signed lift + gate decisions are computed by scripts/checks/check_ranking_lift.py.",
        ],
    }
