...
def _load_parse_audit_verdict():
    ...
    invocation `python scripts/checks/check_audit_artifact_consistency.py …`).
    A package-style invocation `python -m
    scripts.checks.check_audit_artifact_consistency` or import-from-package
    ...
    target = scripts_dir / "audit" / "parse_audit_verdict.py"
...
def run_checks(ctx: LintContext) -> list[LintError]:
    ...
    if ctx.jsonl_events is not None:
        ...
        if v_status != "AUDIT_FAILED":
            ...
            # scripts.checks.check_audit_artifact_consistency`, or imported as
            # `scripts.checks.check_audit_artifact_consistency` from elsewhere).
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.mode is None:
        ...
        # scripts/audit/audit_snapshot.py and scripts/run_codex_audit.sh.
