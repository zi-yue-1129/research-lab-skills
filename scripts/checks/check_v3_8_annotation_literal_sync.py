...
The 8-row finalizer matrix in `scripts/audit/claim_audit_finalizer.py` emits five
...
1. `scripts/audit/claim_audit_finalizer.py` as `ANNOTATION_HIGH_WARN_*` constants
...
FINALIZER_MODULE = REPO_ROOT / "scripts" / "audit" / "claim_audit_finalizer.py"
...
def _extract_finalizer_high_warn_constants(source: str) -> dict[str, str]:
    ...
    Imports `scripts.audit.claim_audit_finalizer` and reads each
    ...
    try:
        from scripts.audit import claim_audit_finalizer  # noqa: F401  (import for side effect: load constants)
