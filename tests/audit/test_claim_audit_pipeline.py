...
repo convention, tests live under `tests/<group>/test_*.py` (CI uses
`python -m unittest tests.<group>.test_*`); we keep the spec-named stem.

Run:
    python -m unittest tests.audit.test_claim_audit_pipeline -v
"""
...
_CAR_SCHEMA = load_json_schema(
    Path(__file__).resolve().parents[2] / "shared/contracts/passport/claim_audit_result.schema.json"
)
...
_CV_SCHEMA = load_json_schema(
    Path(__file__).resolve().parents[2] / "shared/contracts/passport/constraint_violation.schema.json"
)
