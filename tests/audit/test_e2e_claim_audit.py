...
Acceptance:
  ...
  - Scenario B (c3 corrected to SUPPORTED + c5 rewritten with hedged
    ...
    pinned by tests/audit/test_claim_audit_pipeline.py T-P10).
...
Repo convention places tests under `tests/<group>/test_*.py` (CI runs
`python -m unittest tests.<group>.test_*`); the spec name `tests/test_e2e_claim_audit.py`
maps to this file unchanged.

Run:
    python -m unittest tests.audit.test_e2e_claim_audit -v
"""
...
# of a silently green "skip". The earlier tests/audit/test_claim_audit_*.py
...
# ---------------------------------------------------------------------------
# Synthetic 5-citation paper fixture (inline; matches tests/audit/test_claim_audit_*
...
class _E2ETestBase(unittest.TestCase):
    ...
    tests/audit/test_claim_audit_pipeline.py::_PipelineTestBase) is removed
