...
6. `.github/workflows/spec-consistency.yml` contains a direct
   `pytest tests/<group>/test_*.py` invocation outside the runner — including
...
def test_direct_pytest_invocation_in_workflow_fails(workdir: Path) -> None:
    """Drift guard: spec-consistency.yml must NOT have `pytest tests/<group>/test_*.py` outside runner."""
    manifest = workdir / "scripts" / "_ci_pytest_manifest.toml"
    _write(manifest, """
    [[pytest]]
    id = "real-a"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # quoted + flag combo
        '          - run: pytest -v "scripts/test_real_a.py"',
        # current home of these tests: tests/<group>/. The scripts/ variants above
        # stay because the guard still covers that legacy location.
        "          - run: pytest tests/checks/test_real_a.py",
        "          - run: pytest -v tests/audit/test_real_a.py",
        '          - run: python3 -m pytest ./tests/tooling/test_real_a.py',
    ],
    ids=[
        "flag-first-short",
        "flag-first-long",
        ...
        "grouped-tests-dir",
        "grouped-tests-dir-with-flag",
        "grouped-tests-dir-relative-prefix",
    ],
)
