...
- **內容太少時會自己放大**：token 裡的字級是**下限**不是定值，所以內容不多的那一頁會等比放大到填滿版面，剩下的空白平均分到四周，而不是讓圖縮在左上角

## 每一頁各自放大

縮放是**逐頁**決定的，不是整份簡報一個倍率。上面投影片版的第一張只有一個區段，被放大到 1.33 倍；第二張本來就滿了，維持 token 原尺寸。如果用同一個倍率，滿的那一頁會把空的那一頁一起壓回原尺寸——那正是「小圖漂在空白投影片中間」的來源。

倍率用二分搜尋找：每試一個倍率就**真的重排一次**（放大會讓框變寬、列重新換行，高度不是線性的），能塞下就往上，塞不下就往回。上限 2.5 倍，下限恆為 1——字級只會往上，永遠不會為了塞下而縮到下限以下；真的塞不下就多開一頁。

標題、副標與註腳不跟著縮放：它們的基線是對齊安全區固定的，跟著放大會把註腳的下緣頂出安全區（樣式閘門會直接擋下來）。
