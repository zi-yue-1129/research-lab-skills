...
- **`advisor-writing-style`: new skill** — a prescriptive style guide for drafting or revising academic English in a lab advisor's own voice, distilled from a sentence-level diff of the advisor's real manuscript edits against their Markdown source. Ships with `scripts/docx_text.py` and `scripts/sentence_diff.py`, which extract a returned `.docx`'s text and align it against the sources so the profile can be recalibrated whenever the advisor sends back a new round of edits. Ships with one worked example (`references/derivation_evidence.md`); every rule is a calibration against one advisor's own edits, not a universal house style, and the skill documents how to rebuild the reference file for a different advisor. Added to the `academic-research-skills` plugin, alongside `academic-paper`.
