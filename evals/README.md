...
The multi-task harness `scripts/run_evals.py` discovers every `gold/<task>/manifest.yaml`, measures each task, and emits a report shaped by `shared/schemas/evals_lift_report.schema.json`:
