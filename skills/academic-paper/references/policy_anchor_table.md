...
**Lint contract**: `scripts/checks/check_policy_anchor_table.py` enforces 4-anchor coverage, 16-field-per-anchor canonical order, source_strength enum membership, and verbatim-quote presence for mandate/recommend/conditional cells. Mutation tests live at `tests/checks/test_check_policy_anchor_table.py`.
