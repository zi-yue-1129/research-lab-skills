...
- **Location:** `.research/presentations/<deck_id>/` at the project root (same `find .git ancestor` root-finding convention `agent-state` already uses), auto-`.gitignore`d the same way. Reusing the `.research/` top-level name (rather than inventing a new dot-directory) keeps exactly one "internal workflow bookkeeping lives here, never commit it" convention in this repo instead of two.
