...
3. `## Decomposition Procedure` — from the Slide Specification's `complexity_signals` and `layout_regions`, identify semantically independent pieces; assign each a `route` (`native|data|generative|hybrid`, same enum used throughout this skill) and `module_type` (`data_visualization|architecture|conceptual|annotation` — matching one of the four Task 7-10 worker agents exactly); name `input_anchors`/`output_anchors` (stable connection points other modules or the integration step attach to) and `dependencies` (module ids — `validate_visual_module.py` only type-checks `dependencies` as a list, it does **not** referentially validate the ids or reject self-dependencies; the agent must verify both itself, since nothing downstream catches a dangling or self-referencing dependency automatically — `connections[].from`/`.to` endpoints are the only ids the validator actually cross-checks against the declared module set); set `reuse_of` when a module is identical to one already produced elsewhere in the deck (found via manifest search, same reuse-identity discipline as the rest of this skill), else `null`.
