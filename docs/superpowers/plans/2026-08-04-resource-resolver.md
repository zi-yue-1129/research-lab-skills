...
- `resolve.py` never creates a directory unless `--create` is passed on `--set`, and `--set` is only ever called after a human has confirmed a specific path — no autonomous directory creation, moving, or reorganizing of user files anywhere in this plan.
