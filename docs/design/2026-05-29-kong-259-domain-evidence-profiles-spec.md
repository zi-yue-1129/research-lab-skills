...
**Test/CI edits (required, separate from the 3 product edits — see Test strategy for content):** a new `scripts/check_domain_evidence_profile.py` lint, a new `tests/checks/test_check_domain_evidence_profile.py` mutation suite, and a one-line wiring into `.github/workflows/spec-consistency.yml`. These are implementation artifacts the plan MUST deliver; they are not counted among the 3 product edits because they ship no runtime behavior, but an implementer must not skip them.
